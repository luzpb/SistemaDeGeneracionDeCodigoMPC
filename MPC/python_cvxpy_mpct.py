...
for k in range(N):
    ...
    costlist += cp.quad_form(x[:, k] - xref[k-1], Q)  # Penalize deviation from state reference
    costlist += cp.quad_form(u[:, k] - uref[k-1], R)  # Penalize deviation from control reference
...
costlist += cp.quad_form(x[:, N] - xref[N], P)  # Penalize deviation from terminal state reference
