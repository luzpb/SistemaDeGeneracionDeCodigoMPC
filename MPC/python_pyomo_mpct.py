...
# Referencias
xref = np.array({{xref}})  # Referencia para el estado
uref = np.array({{uref}})  # Referencia para la entrada de control
...
def cost_rule(model):
    cost = 0
    for t in model.t:
        cost += sum((model.x[i, t] - xref[i, 0]) * P[i, j] * (model.x[j, t] - xref[j, 0]) for i in range(nx) for j in range(nx))
        cost += (model.u[0, t] - uref[0, 0]) * R[0, 0] * (model.u[0, t] - uref[0, 0])
    return cost
